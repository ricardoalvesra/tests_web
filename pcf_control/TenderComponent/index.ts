...
export class TenderComponent
    ...
    public init(
        ...
        this._container.classList.add("TenderComponent_Container");
    ...
    private renderCreateExample() {
        ...
        let headerDiv: HTMLDivElement = this.createHTMLDivElement(
            "create_container",
            true,
            "Click to create " + TenderComponent._entityName + " record"
        );
    ...
    private renderDeleteExample(): void {
        ...
        let headerDiv: HTMLDivElement = this.createHTMLDivElement(
            "delete_container",
            true,
            "Click to delete " + TenderComponent._entityName + " record"
        );
    ...
    private renderFetchXmlRetrieveMultipleExample(): void {
        ...
        let statusDivHeader: HTMLDivElement = this.createHTMLDivElement(
            ...
            TenderComponent._currencyAttributeNameFriendlyName
        );
    ...
    private createButtonOnClickHandler(event: Event): void {
        ...
        //);
        let currencyAttributeValue: Number = 100;
        // Generate unique record name by appending timestamp to _requiredAttributeValue
        let recordName: string =
            TenderComponent._requiredAttributeValue + "_" + Date.now();
        ...
        var data: any = {};
        data[TenderComponent._requiredAttributeName] = recordName;
        data[TenderComponent._currencyAttributeName] = currencyAttributeValue;
        // store reference to 'this' so it can be used in the callback method
        var thisRef = this;
        // Invoke the Web API to creat the new record
        this._context.webAPI.createRecord(TenderComponent._entityName, data).then(
            function (response: ComponentFramework.EntityReference) {
                ...
                let resultHtml: string =
                    "Created new " + TenderComponent._entityName + " record with below values:";
                resultHtml += "<br />";
                resultHtml += "<br />";
                resultHtml += "id: " + id;
                resultHtml += "<br />";
                resultHtml += "<br />";
                resultHtml += TenderComponent._requiredAttributeName + ": " + recordName;
                resultHtml += "<br />";
                resultHtml += "<br />";
                resultHtml +=
                    TenderComponent._currencyAttributeName + ": " + currencyAttributeValue;
    ...
    private deleteButtonOnClickHandler(): void {
        ...
        var lookUpOptions: any = {
            entityTypes: [TenderComponent._entityName]
        };
        // store reference to 'this' so it can be used in the callback method
        var thisRef = this;
        ...
        lookUpPromise.then(
            ...
            (data: ComponentFramework.EntityReference[]) => {
                if (data && data[0]) {
                    ...
                    let id: string = data[0].id.toString();
                    ...
                    this._context.webAPI.deleteRecord(entityType, id).then(
                        function (response: ComponentFramework.EntityReference) {
                            ...
                            let responseId: string = response.id.toString();
    ...
    private calculateAverageButtonOnClickHandler(): void {
        ...
        fetchXML += "<entity name='" + TenderComponent._entityName + "'>";
        fetchXML +=
            "<attribute name='" +
            TenderComponent._currencyAttributeName +
        ...
        this._context.webAPI
            .retrieveMultipleRecords(TenderComponent._entityName, "?fetchXml=" + fetchXML)
            .then(
    ...
    private refreshRecordCountButtonOnClickHandler(): void {
        ...
        let queryString: string =
            ...
            TenderComponent._currencyAttributeName +
            "&$filter=contains(" +
            TenderComponent._requiredAttributeName +
            ",'" +
            TenderComponent._requiredAttributeValue +
            "')";
        // store reference to 'this' so it can be used in the callback method
        var thisRef = this;
        // Invoke the Web API Retrieve Multiple call
        this._context.webAPI
            .retrieveMultipleRecords(TenderComponent._entityName, queryString)
            .then(
                function (response: any) {
                    ...
                    for (let entity of response.entities) {
                        ...
                        let value: Number = entity[TenderComponent._currencyAttributeName];
